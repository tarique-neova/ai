...
class AWSAIManager:
    ...
    def generate_response(self, prompt):
        input_ids = self.tokenizer.encode(prompt, return_tensors="pt", max_length=512, padding='max_length', truncation=True)
        attention_mask = input_ids.ne(self.tokenizer.pad_token_id)
        output = self.model.generate(input_ids, attention_mask=attention_mask, max_new_tokens=100, num_return_sequences=1, temperature=0.7)
    ...
    def parse_command(self, command):
        ...
        username_match = re.search(r'(?:username|user)\s?(\w+)', command)
        username = username_match.group(1) if username_match else None

        permissions_match = re.search(r'with\s+(read|write|none)\s+s3\s+permissions', command)
        permissions = permissions_match.group(1) if permissions_match else None

        # Extract action
        action = None
        if 'create' in command and 'user' in command:
            action = 'create_user'
